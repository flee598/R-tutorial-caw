...
`summary`, `length`, `head`, `str`, `min`, `max`, `median`, `mean`, `sd`, `abs`,
`as.numeric`, `as.integer`, `as.charactor`, `as.factor`, `cbind`, `class`, `rbind`, `c`,
`floor`, `ceiling`, `round`, `is.na`, `na.omit`, `ncol`, `nrow`, `order`, `paste`,
`quantile`, `rep`, `row.names`, `col.names`, `readRDS`, `saveRDS`, `seq`, `sort`,
`sample`, `round`, `unique`, `data.frame`, `vector`, `list`, `matrix`.
...
2.  sort `my_vec` to be in descending order and save to `my_vec`
3.  sample 3 number from `my_vec`, with replacement, and save in a new object called `my_sample`
